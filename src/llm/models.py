...
class LLMModel(BaseModel):
    ...
    def is_lm_studio(self) -> bool:
        """Check if the model is served by LM Studio"""
...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    if model_provider == ModelProvider.GROQ:
        ...
        if not api_key:
            print(f"API Key Error: Please set ANTHROPIC_API_KEY in your .env file.")
            raise ValueError("Anthropic API key not found. Set ANTHROPIC_API_KEY in your .env file. Get one at https://console.anthropic.com")
